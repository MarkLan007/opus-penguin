...
public class HumanPlayer extends Player implements PlayerInterface {
	...
	//	Note: pid not managed here. Done in CardGame.java.
	
	// HumanPlayer - streamlined just set the usersession for callbacks
	HumanPlayer(//int pid, 
			UserSession us
			//NIOClientSession nioAccessMethods
			//GameInterface gameInterfaceCallbacks
			) {
	...
	void write(String smsg) {
		...
		WsServer.send(userSession, smsg);
		//us.send
		}
}
