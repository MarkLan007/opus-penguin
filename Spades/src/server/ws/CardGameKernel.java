...
public class CardGameKernel {
	 ...
	 *  
	 *  2/28/20 - call dequeue one or more times and process messages
	 *  set pause to be true when there is nothing to do
	 *  i.e. someone needs to call resume if the kernel ever goes idle... (i.e. enqueue)
	 *  
	 *  This is probably not the best way to do this. Instead, see step()
	 *  Ok, so the difference between pause and step is that step only pulls at most one
	 *  message from the queue. resume does till queue is empty, then sets idle
	 */
	void resume() {
		kernelPause = false;
		try {
			while (true) {
				mutexQ.acquire(); // P()
				/*
				 * Access the queue, and release before processing the message. i.e. allow
				 * access to the queue by other processes here... Suppose the process calls
				 * enqueue? That's fine
				 */
				ProtocolMessage pm = null;
				if (messageQueue.size() > 0)
				...
				if (pm != null) {
					/*
					 * There can only be one thread processing moves at a time; this is
					 * independently of the queue access. Anyone who accesses g.process must defend
					 * it with P-s and V-s Maybe process should just do this...
					 */
					g.process(pm);
				} else { // nothing to do. Break out and leave
					break;
				}
			}
		} catch (InterruptedException e) {
		...
		kernelPause = true; // so it can be restarted with new messages
